...
interface ModalProps {
  ...
  onClose: () => void;
  message: string;
  title?: string; // Menambahkan prop title yang opsional
}

const Modal: React.FC<ModalProps> = ({ isOpen, onClose, message, title = "Informasi" }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50">
      <div className="bg-black/60 p-4 sm:p-6 rounded-2xl backdrop-blur-lg ring-1 ring-yellow-500/20 w-full sm:max-w-md mx-auto">
        <h3 className="text-lg font-medium text-yellow-400 mb-4">{title}</h3>
